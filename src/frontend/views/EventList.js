    ...
    const filteredEvents = events.filter((event) => {
        ...
        event.predictions.forEach((prediction) => {
            prediction.scores.forEach((score) => {
                if (
                    !shouldInclude &&
                    score.label.toLowerCase().includes(filterData.label) &&
                    score.score > filterData.score
                ) {
                    shouldInclude = true;
                }
            });
            ...
            <Container className={classes.cardGrid}>
                <Grid container spacing={4}>
                    {filterEvents(events, filterData).map((event) => (
                        <Grid
                            item
                            key={event.videoStream.concat(event.timestamp)}
                            xs={12}
                            sm={6}
                            md={4}
                        >
                            <EventCard
                                event={event}
                                setModalOpenEvent={setOpenModalEvent}
                            />
                        </Grid>
                    ))}
                </Grid>
